...
def buyer_scores(item):
    ...
    story=personal>=2 or len(text)>650
    ...
    credibility=64+(12 if money else 0)+(10 if story else 0)+(8 if personal>=2 else 0)-min(30,seller_hits*8)
    fit=50+(25 if market_for(text)!="unknown" else 0)+(10 if money else 0)+(5 if ("golden visa" in text or "residency by investment" in text) else 0)
    intent=max(0,min(100,intent)); credibility=max(0,min(100,credibility)); fit=max(0,min(100,fit))
    if intent>=78 and credibility>=78 and fit>=70 and (money or transaction>=2 or story): label="HOT"
    elif intent>=60 and credibility>=65 and fit>=60: label="WARM"
...
def keep_candidate(item):
    ...
    if contains_any(text,NEGATIVE_PHRASES) or contains_any(text,["for rent","kiralık","сдам","сдается"]): return False
    seller_hits=sum(1 for p in EXCLUDE_PHRASES if p.lower() in text)
