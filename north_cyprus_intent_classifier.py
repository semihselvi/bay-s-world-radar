...
TENANT_PATTERNS = [
    ...
    r"\bнужн(?:а|ы)\b.{0,80}\b(?:в\s+аренду|на\s+долгий\s+срок|на\s+\d+\s+(?:дн|недел|месяц))",
    r"\bищу\s+(?:квартир\w*|апартамент\w*|дом\w*|вилл\w*|студи\w*|\d\s*\+\s*\d)\b.{0,120}\b(?:долгосроч|долгий\s+срок|помесяч|на\s+месяц|аренд)",
    r"\bищу\b.{0,100}\b(?:долгий\s+срок|долгосроч|помесяч|с\s+\d{1,2}\s+по\s+\d{1,2})\b",
]
...
RENT_CONTEXT_PATTERNS = [
    r"\bfor\s+rent\b", r"\bkiral[ıi]k\b", r"\bсда[её]тся\b", r"\bаренд\w*\b", r"\bmonthly\b", r"\bдолгосроч\w*\b",
]

LONG_TERM_PATTERNS = [
    r"\blong[\s-]?term\b", r"\bmonthly\b", r"\bfor\s+\d+\s+months?\b",
    r"\buzun\s+d[öo]nem\b", r"\bayl[ıi]k\s+[öo]deme\b",
    r"\bдолгосроч\w*\b", r"\bдолгий\s+срок\b", r"\bпомесячн\w*\b", r"\bна\s+(?:год|\d+\s+месяц)",
]
SHORT_TERM_PATTERNS = [
...
def _property_type(text: str) -> str:
    room = re.search(r"\b([0-6]\s*\+\s*[0-3])\b", text)
    kinds = []
    mapping = [
    ...
    bits = []
    if room:
        bits.append(room.group(1).replace(" ", ""))
    if kinds:
        bits.append("/".join(dict.fromkeys(kinds)))
    return " ".join(bits)
...
def classify_intent(item: dict[str, Any]) -> dict[str, Any]:
    ...
    if ambiguous_demand and property_signal and nc_signal and _matches(own, OWNER_DIRECT_PATTERNS):
        return _result(BUYER, [], 72, ["owner_direct_housing_demand", "property_context", "north_cyprus_context"], req)
    ...
    if ambiguous_demand and property_signal and nc_signal:
        return _result(UNKNOWN, [], 58, ["housing_demand_direction_ambiguous"], req)
