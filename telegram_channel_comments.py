...
async def _collect():
    ...
    if not api_id or not api_hash or not session:
        print("TELEGRAM_COMMENTS_DISABLED missing TELEGRAM_API_ID/API_HASH/STRING_SESSION")
        return []
    ...
    for username in NC_PARENT_CHANNELS + _extra_channels():
        ...
        if username and username.lower() not in {x.lower() for x in channels}:
    ...
    if not await client.is_user_authorized():
        print("TELEGRAM_COMMENTS_DISABLED session_not_authorized")
    ...
    try:
        for idx, username in enumerate(channels, 1):
            try:
                ...
                if not linked_id:
                    print(f"TELEGRAM_COMMENT_CHANNEL [{idx}/{len(channels)}] @{username} no_linked_chat")
                    continue
                discussion = None
                ...
                if discussion is None:
                    try:
                        discussion = await client.get_entity(int(linked_id))
                    except Exception:
                        pass
                if not isinstance(discussion, Channel) or not getattr(discussion, "megagroup", False):
                    continue

                linked_found += 1
                parent_title = getattr(parent, "title", "") or username
                discussion_title = getattr(discussion, "title", "") or "discussion"
                count = 0
                async for msg in client.iter_messages(discussion, limit=message_limit):
                    ...
                    if not text:
                        continue
                    url = _chat_link(discussion, msg.id)
                    items[url] = {
                        "source": "Telegram Channel Comments", "url": url,
                        "title": f"Telegram Comments | {parent_title} / {discussion_title} | North Cyprus",
                        "text": text, "published": dt.astimezone(timezone.utc).isoformat(),
                        "author": _sender_name(sender), "source_bucket": "telegram_channel_comments_north_cyprus",
                        "telegram_chat": f"{discussion_title} | North Cyprus",
                        "telegram_parent_channel": f"@{username}",
                    }
                    count += 1
                print(f"TELEGRAM_COMMENT_CHANNEL [{idx}/{len(channels)}] @{username} linked={discussion_title!r} recent_human_messages={count}")
            except FloodWaitError as exc:
                print(f"TELEGRAM_COMMENT_FLOOD_WAIT @{username} seconds={exc.seconds}")
                break
            except Exception as exc:
                print(f"TELEGRAM_COMMENT_CHANNEL_ERROR @{username} {exc}")
    finally:
        await client.disconnect()
...
def collect_channel_comments():
    try:
        ...
        print("TELEGRAM_COMMENTS_EXCEPTION", exc)
        return []
