...
def observe(item, lead, reason):
    if os.getenv("NC_QUERY_LEARNING", os.getenv("NC_SOURCE_LEARNING", "1")).strip() != "1":
        return
    query = str(item.get("telegram_query") or "").strip()
    if not query or query in ("public_group_discovery",):
        return
    key = _norm(query)
...
def ranked_queries(queries, limit, core=None, exploration_ratio=0.30):
    ...
    if len(core_list)>=limit:
        return core_list[:limit]

    candidates=[q for q in unique if _norm(q) not in core_keys]
    remaining=limit-len(core_list)
    ...
    chosen=ranked[:exploit]
    rest=[q for q in candidates if q not in chosen]
    if rest and explore:
        now=datetime.now(timezone.utc); slot=now.timetuple().tm_yday*8+now.hour//3; start=(slot*explore)%len(rest)
        chosen.extend(rest[(start+i)%len(rest)] for i in range(min(explore,len(rest))))
    # Fill if exploration had fewer candidates than requested.
